...
void Cavern::initializeAlienStack() {
    ...
    // Find the highest level of UNKNOWN creatures in the cavern
    int highestLevel = 0;
    std::vector<int> highestLevelIndices;
    for (int i = 0; i < getCurrentSize(); ++i) {
        Creature* creature = items_[i];
        if (creature->getCategory() == "ALIEN" && creature->getLevel() > highestLevel) {
            //If creature is ALIEN and level > highestLevel
            highestLevel = creature->getLevel(); //Set new highestLevel
            highestLevelIndices.clear(); // Reset the list for a new highest level
            highestLevelIndices.push_back(i); //Remember position of creature
        
        } else if (creature->getCategory() == "ALIEN" && creature->getLevel() == highestLevel) {
            //If creature is ALIEN and level = highestLevel
            highestLevelIndices.push_back(i); //Remember position of creature
        }
    }

    for (int index : highestLevelIndices){

      //Push items_ to alien_stack_ at the correct index
      alien_stack_.push(items_[index]);
...
void Cavern::initializeUndeadStack() {
   ...
   //Find highest level UNDEAD creatures
    int highestLevel = 0;
    std::vector<int> highestLevelIndices;
    for (int i = 0; i < getCurrentSize(); ++i) {
        Creature* creature = items_[i];
        if (creature->getCategory() == "UNDEAD" && creature->getLevel() > highestLevel) {
            //If UNDEAD and level > highestLevel
            highestLevel = creature->getLevel();
            highestLevelIndices.clear(); // Reset the list for a new highest level
            highestLevelIndices.push_back(i);
        
        } else if (creature->getCategory() == "UNDEAD" && creature->getLevel() == highestLevel) {
          //UNDEAD with a level = to highestLevel
            highestLevelIndices.push_back(i);
        }
    }

    for (int index : highestLevelIndices){
      //Push creature to undead_stack_ in the correct place
      undead_stack_.push(items_[index]);
...
void Cavern::initializeMysticalStack() {
    ...
    for (int i = 0; i < getCurrentSize(); ++i) {
        Creature* creature = items_[i];
        if (creature->getCategory() == "MYSTICAL" && creature->getLevel() > highestLevel) {
            highestLevel = creature->getLevel();
            highestLevelIndices.clear(); // Reset the list for a new highest level
            highestLevelIndices.push_back(i);
        } else if (creature->getCategory() == "MYSTICAL" && creature->getLevel() == highestLevel) {
            highestLevelIndices.push_back(i);
        }
    }

    for (int index : highestLevelIndices){
      //Push creature to mystical_stack_ in correct place
      mystical_stack_.push(items_[index]);
...
void Cavern::initializeUnknownStack() {
    ...
    for (int i = 0; i < getCurrentSize(); ++i) {
        Creature* creature = items_[i];
        if (creature->getCategory() == "UNKNOWN" && creature->getLevel() > highestLevel) {
            highestLevel = creature->getLevel();
            highestLevelIndices.clear(); // Reset the list for a new highest level
            highestLevelIndices.push_back(i);
        } else if (creature->getCategory() == "UNKNOWN" && creature->getLevel() == highestLevel) {
            highestLevelIndices.push_back(i);
        }
    }
    ...
    for (int index : highestLevelIndices){
      ...
      //Push creature to unknown_stack_
      unknown_stack_.push(items_[index]);
...
*/
void Cavern::setAttacks(std::stack<Creature*> creatures_stack){
      
      std::stack<Creature*> temp_stack;
      std::stack<Creature*>* category_stack = nullptr;
      
      
        
        while (!creatures_stack.empty()) {
          
          Creature* creature = creatures_stack.top();
          creatures_stack.pop(); 
        
          std::cout << "SELECT 2 ATTACKS FOR " << creature->getName() << std::endl;
          ...
          for (int i = 0; i < 2; ++i) {
           ...
           temp_stack.push(creature);

         }
         if (!temp_stack.empty()) {

          //Clear appropriate stack before pushing creature
        if (temp_stack.top()->getCategory() == "ALIEN") {
            clearAlienStack();
            category_stack = &alien_stack_;
        } else if (temp_stack.top()->getCategory() == "UNDEAD") {
            clearUndeadStack();
            category_stack = &undead_stack_;
        } else if (temp_stack.top()->getCategory() == "MYSTICAL") {
            clearMysticalStack();
            category_stack = &mystical_stack_;
        } else if (temp_stack.top()->getCategory() == "UNKNOWN") {
            clearUnknownStack();
            category_stack = &unknown_stack_;
        } else {
            return;
        }
    }
        while (!temp_stack.empty()) {
            ...
            //Push creatures from temp_stack to correct stack 
            Creature* tempcreature = temp_stack.top();
          
            category_stack->push(tempcreature);
            temp_stack.pop();    
            } 
}
