...
struct Projectile{
    ...
    int quantity_;
        ...
        */
        Mindflayer(const std::string& name, Category category = ALIEN, int hitpoints = 1, int level = 1, bool tame = false, std::vector<Projectile> projectiles = std::vector<Projectile>(), bool summoning = false, std::vector<Variant> affinities = std::vector<Variant>());
...
*/
        void addAttack(const int&) override;
